...
TOKEN_COUNT=$(curl --silent --request GET --url http://testuser:password@gitea.${MINIKUBE_IP}.nip.io/api/v1/users/testuser/tokens | jq '. | length')

if (( ${TOKEN_COUNT} == 0 )); then
  TOKEN=$(curl -XPOST -H "Content-Type: application/json" -k -d '{"name":"machine-access"}' -u testuser:password http://gitea.${MINIKUBE_IP}.nip.io/api/v1/users/testuser/tokens | jq --raw-output .sha1)
